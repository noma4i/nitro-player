...
var _playerFactory = require("./playerFactory.js");
...
class NitroPlayer extends _NitroPlayerEvents.NitroPlayerEvents {
  ...
  constructor(source) {
    const player = (0, _playerFactory.createPlayer)(source);

    // Initialize events
    super(player.eventEmitter);
    this._player = player;
    this._source = (0, _prepareSource.prepareSource)(source);
  }
  ...
  parseError(error) {
    return (0, _NitroPlayerError.tryParseNativeNitroPlayerError)(error);
  }
  ...
  refreshMemorySize() {
    ...
    _reactNativeNitroModules.NitroModules.updateMemorySize(this._player);
    _reactNativeNitroModules.NitroModules.updateMemorySize(this._player.source);
  }
  runSync(operation) {
    try {
      return operation();
  ...
  async replaceSourceAsync(source) {
    const nextSource = (0, _prepareSource.prepareSource)(source);
    if (this._source?.identity.playbackKey === nextSource.identity.playbackKey) {
      return;
    }
    this.refreshMemorySize();
    await this.runAsync(() => this.player.replaceSourceAsync((0, _sourceFactory.createNativeNitroSource)(nextSource)));
