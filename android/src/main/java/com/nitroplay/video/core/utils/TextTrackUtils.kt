...
object TextTrackUtils {
    fun getAvailableTextTracks(player: ExoPlayer, source: HybridNitroPlayerSourceSpec): Array<TextTrack> {
    ...
    fun selectTextTrack(
        player: ExoPlayer,
        textTrack: TextTrack?,
        source: HybridNitroPlayerSourceSpec,
    ...
    fun getSelectedTrack(player: ExoPlayer, source: HybridNitroPlayerSourceSpec): TextTrack? {
        return Threading.runOnMainThreadSync {
            val currentTracks = player.currentTracks
            var globalTrackIndex = 0
