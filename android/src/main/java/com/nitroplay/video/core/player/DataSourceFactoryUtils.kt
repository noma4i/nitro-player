...
fun buildBaseDataSourceFactory(context: Context, source: HybridNitroPlayerSourceSpec): DefaultDataSource.Factory {
...
fun buildHttpDataSourceFactory(context: Context, source: HybridNitroPlayerSourceSpec): OkHttpDataSource.Factory {
  val client = OkHttpClientProvider.getOkHttpClient()

  if (context is ReactContext) {
    (client.cookieJar as? CookieJarContainer)?.let { container ->
