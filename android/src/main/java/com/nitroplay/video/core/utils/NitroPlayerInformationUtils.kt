...
object NitroPlayerInformationUtils {
  fun fromUri(uri: String, headers: Map<String, String> = emptyMap()): NitroPlayerInformation {
    ...
    // Get file size
    val fileSize = NitroPlayerFileHelper.getFileSizeFromUri(uri)

    val videoInfo = NitroPlayerInformation(
      bitrate = bitrate,
      width = width,
      height = height,
      duration = duration,
      ...
      isHDR = isHDR,
      isLive = isLive,
      orientation = NitroPlayerOrientationUtils.fromWHR(width.toInt(), height.toInt(), rotation)
    )
