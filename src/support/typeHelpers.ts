...
type CheckExactUnion<T extends string, A extends readonly T[]> =
  [T] extends [A[number]] ? A : ['Missing union members', Exclude<T, A[number]>];
 ...
 */
export function unionTuple<T extends string>() {
  return <A extends readonly T[]>(...members: A): CheckExactUnion<T, A> => {
    return members as CheckExactUnion<T, A>;
  };
}
